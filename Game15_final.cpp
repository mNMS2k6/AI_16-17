...
void creat_childs(data child, string keychild, char dir)
{
  if (is_Solution(keychild))
	...
	case 'd':
      ...
      print(child->config);
      cout << "###FINAL CONFIG###" << endl << endl;
      cout << "Depth: " << child->depth << endl;
      cout << "Path from the 1st config:  " << backtrack(child) << endl;
      cout << "num of nodes (space used): " << table.size() << endl;
...
void general_search_2(data parent, char dir)
{
  if (is_Solution(converter(parent->config)))
	...
	case 'd':
      ...
      cout << "Path from the 1st config:  " << backtrack(parent) << endl;
      cout << "num of nodes (space used): " << table.size() << endl;
